...
class WSGIApp(Setting):
    ...
    default = None
    desc = """\
        A WSGI app path in pattern ``$(MODULE_NAME):$(VARIABLE_NAME)``.
...
class Bind(Setting):
    ...
    desc = """\
        ...
        will bind the `test:app` app on localhost both on ipv6
        and ipv4 interfaces.
...
class Reload(Setting):
    ...
    desc = '''\
        ...
        restarted whenever app code changes.

        The reloader is incompatible with app preloading. When using a
        paste configuration be sure that the server block does not import any
        app code or the reload will not work as designed.
...
class PreloadApp(Setting):
    ...
    default = False
    desc = """\
        Load app code before the worker processes are forked.

        By preloading an app you can save some RAM resources as well as
        speed up server boot times. Although, if you defer app loading
        to each worker process, you can reload your app code easily by
        restarting workers.
        """
...
class SecureSchemeHeader(Setting):
    ...
    desc = """\
        ...
        ``wsgi.url_scheme`` to ``https``, so your app can tell that the
        request is secure.
...
class DefaultProcName(Setting):
    ...
    default = "gunicorn"
    desc = """\
        Internal setting that is adjusted for each type of app.
        """
...
class PostWorkerInit(Setting):
    ...
    desc = """\
        Called just after a worker has initialized the app.
...
class PermitUnconventionalHTTPMethod(Setting):
    ...
    desc = """\
        ...
        changes around the incomplete app of those restrictions.
