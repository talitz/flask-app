...
class CompiledRouter:
    ...
    first routed request) to reduce the time it takes to start the app.
    This may noticeably delay the first response of the app when a large
    number of routes have been added. When adding the last route
    to the app a `compile` flag may be provided to force the router
    to compile immediately, thus avoiding any delay for the first response.

    Note:
        When using a multi-threaded web server to host the app, it is
    ...
    def add_route(self, uri_template, resource, **kwargs):
        ...
        Keyword Args:
            ...
            compile (bool): Optional flag that can be used to compile the
                ...
                request, especially when the app implements a large
