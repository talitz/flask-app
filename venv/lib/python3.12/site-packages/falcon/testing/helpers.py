...
class ASGIWebSocketSimulator:
    ...
    async def close(self, code: Optional[int] = None):
        ...
        Keyword Args:
            code (int): The WebSocket close code to send to the app
    ...
    async def _collect(self, event: Dict[str, Any]):
        ...
        if self._state == _WebSocketState.CONNECT:
            raise falcon_errors.OperationNotAllowed(
                'An ASGI app must receive the first websocket.connect '
        ...
        if self._state == _WebSocketState.HANDSHAKE:
            if event_type == EventType.WS_ACCEPT:
                ...
                raise falcon_errors.OperationNotAllowed(
                    'An ASGI app must send either websocket.accept or '
...
def get_encoding_from_headers(headers):
    ...
    # NOTE(kgriffs): Added checks for text/event-stream and app/json
    if content_type in ('text/event-stream', 'app/json'):
        return 'UTF-8'
...
def create_scope(
    ...
    Keyword Args:
        ...
        root_path (str): The root path this app is mounted at; same as
...
def create_environ(
    ...
    Keyword Args:
        ...
        root_path (str): Value for the ``SCRIPT_NAME`` environ variable, described in
            ...
            app corresponds to the "root" of the server.' (default ``''``)
...
def closed_wsgi_iterable(iterable):
    ...
    WSGI ``start_response`` function is called even if the WSGI app is
    a generator.
